...
const ResultsPage: React.FC = () => {
  ...
  if (!results) {
    return (
      <div className="min-h-screen bg-surface-950 pt-20 text-center text-white">Results not available</div>
    );
  }
  ...
  return (
    <div className="page-wrapper bg-surface-950">
      <div className="page-container">
        <Link to="/dashboard" className="inline-flex items-center gap-2 text-surface-400 hover:text-white mb-6 text-sm transition-colors font-medium">
          <ArrowLeft className="w-4 h-4" /> Back
        </Link>

        {/* Header */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary-500/8 border border-primary-500/15 text-primary-300 text-sm mb-5">
            <BarChart3 className="w-4 h-4" /> Election Results
          </div>
          <h1 className="text-3xl md:text-4xl font-display font-bold text-white mb-3 tracking-tight">{election.title}</h1>
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-12">
          {[
            { icon: Users, color: 'text-primary-400', value: turnout.totalEligible, label: 'Eligible Voters' },
            { icon: TrendingUp, color: 'text-accent-400', value: turnout.totalVotes, label: 'Votes Cast' },
            { icon: BarChart3, color: 'text-amber-400', value: `${turnout.participationRate}%`, label: 'Participation Rate' },
          ].map((stat, i) => (
            <motion.div
              key={stat.label}
            ...
            >
              ...
              <p className={`text-3xl font-display font-bold ${i === 0 ? 'text-white' : stat.color}`}>{stat.value}</p>
        ...
        {facultyTurnout?.length > 0 && (
          <div className="card p-6 mb-12">
            <h2 className="text-lg font-semibold text-white mb-6">Faculty Turnout</h2>
          ...
          return (
            ...
            >
              <h2 className="text-xl font-display font-bold text-white mb-6 flex items-center gap-2.5">
                <Award className="w-5 h-5 text-primary-400" />{position}
              </h2>
              <div className="grid lg:grid-cols-2 gap-6">
                ...
                <div className="space-y-3">
                  {sorted.map((c: any, i: number) => (
                    <div key={c.id} className={`p-4 rounded-xl border ${i === 0 ? 'bg-primary-500/[0.03] border-primary-500/15' : 'card'}`}>
                      <div className="flex items-center justify-between mb-2.5">
                        <div className="flex items-center gap-3">
                          {i === 0 && <Trophy className="w-5 h-5 text-amber-400" />}
                          <span className={`font-semibold ${i === 0 ? 'text-white' : 'text-surface-300'}`}>{c.fullName}</span>
                        </div>
                        <span className="font-display font-bold text-white">
                          {c.voteCount} <span className="text-surface-500 text-sm">({c.percentage}%)</span>
                        </span>
                      </div>
        ...
        {!showResults && (
          <div className="text-center py-16 card">
            <BarChart3 className="w-16 h-16 text-surface-700 mx-auto mb-4" />
            <h3 className="text-xl font-semibold text-surface-400">Results Not Available Yet</h3>
