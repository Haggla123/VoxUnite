...
const ElectionDetail: React.FC = () => {
  ...
  if (!election) {
    return <div className="min-h-screen bg-surface-950 pt-20 text-center text-white">Not found</div>;
  }
  ...
  return (
    <div className="page-wrapper bg-surface-950">
      <div className="page-container">
        <Link to="/admin/elections" className="inline-flex items-center gap-2 text-surface-400 hover:text-white mb-6 text-sm transition-colors font-medium">
        ...
        <div className="card p-6 mb-8">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              <h1 className="text-2xl font-display font-bold text-white tracking-tight">{election.title}</h1>
              <p className="text-surface-400 mt-1.5">{election.description}</p>
              ...
              {election.status === 'active' && (
                <>
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 px-4 py-2 bg-white/[0.04] hover:bg-white/[0.08] text-white rounded-xl text-sm border border-white/[0.08] font-medium transition-all"
                  >
          ...
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="mb-6 card p-6 overflow-hidden">
            <form onSubmit={handleAddCandidate} className="space-y-3">
              <div className="grid md:grid-cols-2 gap-3">
                <input type="text" value={candidateForm.fullName} onChange={e => setCandidateForm(f => ({ ...f, fullName: e.target.value }))} placeholder="Full Name" required className="px-4 py-2.5 bg-white/[0.04] border border-white/[0.08] rounded-xl text-white text-sm focus:outline-none focus:border-primary-500 transition-all" />
                <select value={candidateForm.position} onChange={e => setCandidateForm(f => ({ ...f, position: e.target.value }))} required className="px-4 py-2.5 bg-white/[0.04] border border-white/[0.08] rounded-xl text-white text-sm focus:outline-none focus:border-primary-500 transition-all">
                  <option value="">Select Position</option>
                  {positions.map((p: any) => <option key={p.title} value={p.title}>{p.title}</option>)}
                </select>
              </div>
              <input type="text" value={candidateForm.slogan} onChange={e => setCandidateForm(f => ({ ...f, slogan: e.target.value }))} placeholder="Campaign Slogan" className="w-full px-4 py-2.5 bg-white/[0.04] border border-white/[0.08] rounded-xl text-white text-sm focus:outline-none focus:border-primary-500 transition-all" />
              <textarea value={candidateForm.manifesto} onChange={e => setCandidateForm(f => ({ ...f, manifesto: e.target.value }))} placeholder="Manifesto" rows={3} className="w-full px-4 py-2.5 bg-white/[0.04] border border-white/[0.08] rounded-xl text-white text-sm focus:outline-none focus:border-primary-500 transition-all resize-none" />
              <div className="grid md:grid-cols-2 gap-3">
                <input type="text" value={candidateForm.faculty} onChange={e => setCandidateForm(f => ({ ...f, faculty: e.target.value }))} placeholder="Faculty" className="px-4 py-2.5 bg-white/[0.04] border border-white/[0.08] rounded-xl text-white text-sm focus:outline-none focus:border-primary-500 transition-all" />
                <input type="text" value={candidateForm.department} onChange={e => setCandidateForm(f => ({ ...f, department: e.target.value }))} placeholder="Department" className="px-4 py-2.5 bg-white/[0.04] border border-white/[0.08] rounded-xl text-white text-sm focus:outline-none focus:border-primary-500 transition-all" />
              </div>
        ...
        {Object.entries(grouped).map(([position, cands]) => (
          <div key={position} className="mb-8">
            <h3 className="text-lg font-semibold text-white mb-4 flex items-center gap-2.5">
              <div className="w-2.5 h-2.5 rounded-full bg-primary-500 shadow-sm shadow-primary-500/50" /> {position}
            </h3>
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
              {(cands as any[]).map(c => (
                <div key={c._id} className="card p-5 hover:border-white/10">
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-3">
                      <div className="w-12 h-12 rounded-xl bg-surface-800 flex items-center justify-center shrink-0 overflow-hidden">
                        {c.photo
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-primary-400 truncate">{c.slogan}</p>
                      </div>
                    </div>
                    <button onClick={() => handleDeleteCandidate(c._id)} className="p-2 text-surface-600 hover:text-red-400 transition-colors rounded-lg hover:bg-red-500/10">
                      <Trash2 className="w-4 h-4" />
                    </button>
                  </div>
                  {election.status === 'closed' && (
                    <div className="mt-3 pt-3 border-t border-white/5">
                      <p className="text-sm text-surface-400">{c.voteCount} votes</p>
                    </div>
                  )}
                </div>
              ))}
