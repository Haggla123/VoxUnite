...
router.post('/upload', authenticateAdmin, uploadVoterFile.single('voterFile'), async (req: Request, res: Response) => {
  try {
...
router.get('/faculties', async (_req: Request, res: Response) => {
  try { const faculties = await EligibleVoter.distinct('faculty', { isActive: true }); res.json(faculties); } catch (error) { res.status(500).json({ message: 'Server error' }); }
});
